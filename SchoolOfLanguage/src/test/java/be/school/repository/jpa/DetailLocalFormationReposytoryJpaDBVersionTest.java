...
public class DetailLocalFormationReposytoryJpaDBVersionTest {
	...
	public void testFindAll() {
		List<DetailLocalFormation> list = detailLocalFormationRepository.findAll();
	...
	public void testFindAllByFormateur() {
		DetailLocalFormation detailLocalFormation = detailLocalFormationRepository.findById(1L);
		Formateur formateur = detailLocalFormation.getFormateur();
		
		List<DetailLocalFormation>result = detailLocalFormationRepository.findAllByFormateur(formateur);
		assertEquals(2, result.size());
		assertThat(result.get(0).getFormateur().getLogin(), is("mutanda"));
		assertThat(result.get(0).getFormateur(), allOf(hasProperty("id",is(Long.parseLong("1")))));
	}
	...
	public void testGetParticipantNumber() {
		...
		Long result = detailLocalFormationRepository.getParticipantNumber(local, seance);
